...
import it.cnr.istc.stlab.ontonethub.web.resources.OntonethubIndexingResource;
...
public class IndexingJob implements Job {
	...
	private String ontologyName, ontologyDescription, baseURI, ontologyURI, vocabNs;
	private Model data;
	...
	public IndexingJob(String ontologyName, String ontologyDescription, String baseURI, Model data, BundleContext ctx, TcManager tcManager, File ontologiesFolder, String ontologyURI, String vocabNs) {
	...
	public JobResult call() throws Exception {
		...
		try {
			File folder = new File(stanbolHome + File.separator + OntonethubIndexingResource.RUNNABLE_INDEXER_EXECUTABLES_FOLDER);
			String tempFolderName = "_" + System.currentTimeMillis();
			tempFolder = new File(folder, tempFolderName);
			
			Process initProcess = Runtime.getRuntime().exec("java -jar " + stanbolHome + File.separator + OntonethubIndexingResource.RUNNABLE_INDEXER_EXECUTABLES + " init " + tempFolder.getPath());
			initProcess.waitFor();
		...
		if(!error){
			...
			Process indexingProcess = Runtime.getRuntime().exec("java -jar "  + stanbolHome + File.separator + OntonethubIndexingResource.RUNNABLE_INDEXER_EXECUTABLES + " index " + tempFolder.getPath());
			...
			if(bundleFile.exists() && zippedIndexFile.exists()){
				...
				try{
					...
					IRI jobIRI = new IRI(ontologyURI + "/" + jobId);
